...
const PdfToXml = () => {
    const words = ["Better", "PDF", "Perfect", "XML"];
    ...
    const convertPdfToXml = async (fileUrl: string) => {
        setState('converting');

        try {
            const response = await fetch('https://api.pdf.co/v1/pdf/convert/to/xml', {
                method: 'POST',
                headers: {
                    'x-api-key': API_KEY,
    ...
    return (
        <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6]   flex flex-col items-center  justify-start">
            ...
            <div className='pb-10'>
                ...
                <p className="text-muted-foreground text-lg">Convert your PDF images to high-quality XML documents</p>
                    ...
                    <div className="space-y-6">
                        ...
                        {state === 'converting' && (
                            <div className="flex flex-col items-center space-y-4">
                               <Spinner/>
                                <p className="text-muted-foreground">Converting PDF to XML</p>
                            </div>
                        )}
                        ...
                        {state === 'ready' && (
                            <div className="space-y-4">
                                {convertedFileUrls.map((url, index) => (
                                    <Button
                                        key={index}
                                        onClick={() => downloadFile(url, `converted-xlsx-${index + 1}.xml`)}
