...
const PngToJson = () => {
  const words = ["Better", "JSON", "Perfect", "Png"];
  ...
  const convertImageToJson = async (fileUrl: string) => {
    setState('converting');
    try {
      ...
      if (data.error === false && data.url) {
        ...
        console.error('Image to JSON conversion failed:', data);
        alert(`Image to JSON conversion failed: ${data.message || 'Please try again.'}`);
        setState('select');
      }
    } catch (error) {
      console.error('Convert error:', error);
      alert('Image to JSON conversion failed. Please try again.');
      setState('select');
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-muted-foreground text-lg">Png to JSON Converter</p>
        <p className="text-[#a855f7] text-sm mt-2 font-medium">
          Convert PNG images into JSON format with structured data extraction.
        </p>
      </div>
          ...
          <div className="space-y-6">
            {state === 'select' && !uploadedFileUrl && (
              ...
              >
                ...
                <h3 className="text-xl font-semibold text-white">Choose Png File</h3>
                <input
                  ref={fileInputRef}
                  type="file"
            ...
            {state === 'converting' && (
              <div className="flex flex-col items-center space-y-4">
                <Spinner />
                <p className="text-muted-foreground">Converting Image to JSON...</p>
              </div>
            )}
