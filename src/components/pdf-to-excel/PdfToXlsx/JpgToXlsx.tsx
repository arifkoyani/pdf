...
const JpgToXlsx = () => {
    const words = ["Better", "XLSX", "Perfect", "XLSX"];
    ...
    const convertJpgToXlsx = async (fileUrl: string) => {
        setState('converting');

        try {
            const response = await fetch('https://api.pdf.co/v1/pdf/convert/to/xlsx', {
                method: 'POST',
                headers: {
                    'x-api-key': API_KEY,
            ...
            if (data.error === false) {
                ...
                if (urls.length > 0) {
                    ...
                    setState('select');
                    alert('Conversion failed. No XLSX files generated. Please try again.');
                }
            } else {
                setState('select');
    ...
    return (
        <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6]   flex flex-col items-center  justify-start">
            ...
            <div className='pb-10'>
                ...
                <p className="text-muted-foreground text-lg">Convert your JPG or JPEG images to high-quality XLSX documents</p>
                    ...
                    <div className="space-y-6">
                        ...
                        {state === 'converting' && (
                            <div className="flex flex-col items-center space-y-4">
                                ...
                                <Spinner/>
                                <p className="text-muted-foreground">Converting JPG or JPEG to XLSX</p>
                            </div>
                        )}
                        ...
                        {state === 'ready' && (
                            <div className="space-y-4">
                                {convertedFileUrls.map((url, index) => (
                                    <Button
                                        key={index}
                                        onClick={() => downloadFile(url, `converted-xlsx-${index + 1}.xlsx`)}
