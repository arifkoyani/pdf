...
const RemovePasswordFromPdf = () => {
  const words = ["Better", "Pdf", "Perfect", "Unprotected"];
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (data.error === false) {
        ...
        // ✅ start remove-password automatically
        removePassword(data.url);
      } else {
        setState('select');
  ...
  const removePassword = async (fileUrl: string, password?: string) => {
    setState('converting');
    try {
      const payload: any = {
        url: fileUrl,
      ...
      if (password) {
        payload.password = password; // only if user provides
      }
  
      const response = await fetch('https://api.pdf.co/v1/pdf/security/remove', {
        method: 'POST',
        headers: {
          'x-api-key': API_KEY,
      ...
      if (data.error === false && data.url) {
        ...
        setState('ready');
      } else if (data.error && data.errorCode === 441 && !password) {
        // PDF needs password but not provided
        const userPassword = prompt("This PDF is password protected. Please enter the password:");
        if (userPassword) {
          removePassword(fileUrl, userPassword); // retry with password
        } else {
          setState('select');
          alert("Password is required to unlock this PDF.");
        }
      } else {
        console.error('Remove password failed:', data);
        alert(`Remove password failed: ${data.message || 'Please try again.'}`);
        setState('select');
      }
    } catch (error) {
      console.error('Remove password error:', error);
      alert('Remove password failed. Please try again.');
      setState('select');
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-muted-foreground text-lg">Remove password protection from PDF</p>
        <p className="text-[#a855f7] text-sm mt-2 font-medium">
          Unlock secured PDF files with one click.
        </p>
      </div>
      ...
      <Card className="h-fit p-8 shadow-elegant border-0 backdrop-blur-sm">
        <div className="text-center space-y-6">
          ...
          {state === 'converting' && (
            <div className="flex flex-col items-center space-y-4">
              <Spinner />
              <p className="text-muted-foreground">Removing password from PDF...</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              {convertedFileUrls.map((url, index) => (
                <div key={index} className="flex items-center space-x-2">
                  {/* Download Button */}
                  <Button
                    onClick={() => downloadFile(url, "unprotected.pdf", index)}
                  ...
                  {/* Copy Link Icon Button with feedback */}
                  <Button
                    variant="outline"
                    size="icon"
...
export default RemovePasswordFromPdf;
