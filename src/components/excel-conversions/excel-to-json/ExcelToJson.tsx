...
const ExcelToJson = () => {
  ...
  const convertExcelToJson = async (file: File) => {
    ...
    try {
      const response = await fetch("https://api.pdf.co/v1/xls/convert/to/json", {
        method: "POST",
        headers: {
          "x-api-key": API_KEY,
  ...
  const downloadFile = async (url: string, fileName: string, index: number) => {
    ...
    try {
      ...
      link.href = objectUrl;
      link.download = fileName || "converted-file.json";
    ...
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className="pb-10">
        ...
        <p className="text-muted-foreground text-lg">
          Upload your Excel xls/xlsx file  and convert it to JSON format
        </p>
      </div>
          ...
          {(state === "uploading" || state === "converting") && (
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-muted-foreground">
                {state === "uploading" ? "Uploading file..." : "Converting to Json..."}
              </p>
            </div>
          )}
          ...
          {state === "ready" && convertedFileUrl && (
            <div className="space-y-6">
              <div className="bg-white p-6 rounded-lg shadow-md space-y-4">
                ...
                <p className="text-gray-600">Your Excel file has been successfully converted to JSON format.</p>
                
                <div className="flex items-center space-x-2">
                  <Button
                    onClick={() => downloadFile(convertedFileUrl, "converted-file.json", 0)}
