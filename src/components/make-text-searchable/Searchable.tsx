...
const SearchablePdf = () => {
  const words = ["Searchable", "OCR-Ready", "Text-Enabled", "Indexed"];
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (data.error === false) {
        ...
        processSearchablePdf(data.url, file.name);
      } else {
        setState('select');
        setUploadProgress(0);
  ...
  const processSearchablePdf = async (fileUrl: string, fileName: string) => {
    setState('processing');
    setProcessingMessage('Making PDF searchable with OCR...');
    
    try {
      const response = await fetch('https://api.pdf.co/v1/pdf/makesearchable', {
        method: 'POST',
        headers: {
          'x-api-key': API_KEY,
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-muted-foreground text-lg">Make Text Searchable</p>
          ...
          {state === 'processing' && (
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-sm text-blue-600">
                Running OCR and adding searchable text layer to your PDF...
              </p>
            </div>
          )}
          ...
          {state === 'ready' && searchableResult && (
            <div className="space-y-4">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-green-700 font-medium">✅ Successfully created searchable PDF with OCR!</p>
              </div>
              
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-left">Searchable PDF Result:</h3>
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                  <div className="flex items-center space-x-3">
                    <Search className="w-5 h-5 text-green-500" />
                    <div className="text-left">
                      <p className="font-medium text-sm">{searchableResult.name}</p>
                      <p className="text-xs text-muted-foreground">OCR-enabled searchable PDF</p>
                    </div>
                  </div>
