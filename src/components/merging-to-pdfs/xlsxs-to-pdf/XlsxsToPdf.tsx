...
const XlsxsToPdf = () => {
  ...
  const [processingMessage, setProcessingMessage] = useState('Merging XLS/XLSX Files to PDF...');
  ...
  const getFileIcon = (fileName: string, fileType: string) => {
    ...
    if (['txt'].includes(extension || '') || fileType.includes('image')) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert('Please select a valid image file (Txt)');
      }
    }
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        <div className="h-[4rem] flex justify-center items-center px-4">
          <div className="flex flex-wrap justify-center py-1 items-center mx-auto text-neutral-600 text-2xl sm:text-3xl md:text-4xl lg:text-5xl gap-2">
            Merge XLS/XLSX Files To
            <div className="w-[120px] sm:w-[150px] md:w-[180px] text-left">
              <FlipWords words={words} />
            </div>
          </div>
        </div>
        <p className="text-muted-foreground text-lg">Merge Multiple XLS/XLSX Files into a Single PDF</p>
        <p className="text-[#a855f7] text-sm mt-2 font-medium text-center">
          Combine your XLS/XLSX Files into a single PDF document.
        </p>
      </div>
...
/>
  ...
  Supported: XLS/XLSX (XLS or XLSX Format)
</p>
            </div>
          )}
          ...
          {uploadedFiles.length > 0 && !mergedFileUrl && state !== 'merging' && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-left">Uploaded XLS/XLSX Files ({uploadedFiles.length})</h3>
         ...
         >
           ...
           Merge {uploadedFiles.length} TXT File{uploadedFiles.length > 1 ? 's' : ''} to PDF
         </Button>
              )}
            </div>
          )}
