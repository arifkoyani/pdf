...
const JpgsToPdf = () => {
  ...
  const getFileIcon = (fileName: string, fileType: string) => {
    ...
    if (['jpg', 'jpeg'].includes(extension || '') || fileType.includes('image')) {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert('Please select a valid image file (JPG or JPEG)');
      }
    }
    ...
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-[#a855f7] text-sm mt-2 font-medium text-center">
          Combine your JPG and JPEG images into a single PDF document.
        </p>
      </div>
          ...
          {(state === 'select' || uploadedFiles.length > 0) && !mergedFileUrl && state !== 'merging' && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-muted-foreground">
                Supported: JPG, JPEG (Image Files)
              </p>
            </div>
          )}
