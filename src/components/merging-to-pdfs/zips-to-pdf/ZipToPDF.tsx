...
const ZipToPdf = () => {
  ...
  const [processingMessage, setProcessingMessage] = useState('Mergin ZIP Files to PDF...');
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-muted-foreground text-lg">Merge Multiple ZIP Files into a Single PDF</p>
        <p className="text-[#a855f7] text-sm mt-2 font-medium text-center">
          Combine your ZIP Files into a single PDF document.
        </p>
      </div>
...
/>
  ...
  Supported: ZIP (ZIP Format)
          ...
          {uploadedFiles.length > 0 && !mergedFileUrl && state !== 'merging' && (
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-left">Uploaded ZIP Files ({uploadedFiles.length})</h3>
         ...
         >
           ...
           Merge {uploadedFiles.length} ZIP File{uploadedFiles.length > 1 ? 's' : ''} to PDF
         </Button>
              )}
            </div>
          )}
