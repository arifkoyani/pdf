...
const RtfsToPdf = () => {
  ...
  const getFileIcon = (fileName: string, fileType: string) => {
    ...
    if (['rtf'].includes(extension || '') || fileType.includes('image')) {
  ...
  const isValidFileType = (file: File): boolean => {
    const validTypes = ['application/rtf', 'text/rtf'];
    const validExtensions = ['rtf'];
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert('Please select a valid image file (RTF)');
      }
    }
    ...
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-[#a855f7] text-sm mt-2 font-medium text-center">
          Combine your RTF images into a single PDF document.
        </p>
      </div>
...
/>
  ...
  Supported: RTF (Rich Text Format)
</p>
            </div>
          )}
         ...
         >
           ...
           Merge {uploadedFiles.length} RTF File{uploadedFiles.length > 1 ? 's' : ''} to PDF
         </Button>
              )}
            </div>
          )}
