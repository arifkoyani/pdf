...
const DocToPdf = () => {
  ...
  const [processingMessage, setProcessingMessage] = useState('Merging documents to PDF...');
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (isValidFileType(file)) {
        ...
        alert('Please select a valid file (DOC, DOCX, or PDF)');
      }
    }
  ...
  const mergeDocuments = async () => {
    if (uploadedFiles.length < 1) {
      alert('Please upload at least 1 file to merge');
      return;
    }

    setState('merging');
    setProcessingMessage('Merging documents to PDF...');
    
    try {
      // Create a comma-separated string of all file URLs
      ...
      if (data.error === false && data.url) {
        ...
        console.error('Document merge failed:', data);
        alert(`Document merge failed: ${data.message || 'Please try again.'}`);
        setState('select');
      }
    } catch (error) {
      console.error('Merge error:', error);
      alert('Document merge failed. Please try again.');
      setState('select');
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-65px)] bg-[#fff9f6] flex flex-col items-center justify-start">
      ...
      <div className='pb-10 flex flex-col justify-center items-center'>
        ...
        <p className="text-muted-foreground text-lg">Merge Multiple Documents into a Single PDF</p>
        <p className="text-[#a855f7] text-sm mt-2 font-medium text-center">
          Combine your DOC, DOCX, and PDF files into a single PDF document with perfect formatting.
        </p>
      </div>
          ...
          {(state === 'select' || uploadedFiles.length > 0) && !mergedFileUrl && state !== 'merging' && (
            <div className="space-y-6">
              ...
              <p className="text-sm text-muted-foreground">
                Supported: DOC, DOCX (Microsoft Word Documents), PDF
              </p>
            </div>
          )}
          ...
          {state === 'merging' && (
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-sm text-blue-600">
                This may take a few moments for document conversion and merging...
              </p>
            </div>
          )}
